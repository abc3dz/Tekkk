    ...
    commands
        ...
        .with_children(|root| {
            root.spawn((
                ...
                BackgroundColor(
                    Color::srgba(0.05, 0.05, 0.08, 0.95),
                ),
            ))
            .with_children(|panel| {
                panel.spawn((
...
fn update_element_status_ui(
    ...
    mut value_query: Query<
        ...
        (
            Without<PlayerStatusBonusText>,
            Without<ElementExpText>,
        ),
    >,

    mut bonus_query: Query<
        (
            &PlayerStatusBonusText,
            &mut TextSpan,
        ),
        (
            Without<PlayerStatusValueText>,
            Without<ElementExpText>,
        ),
    >,
    ...
    // ค่าที่เพิ่มขึ้นจาก BaseStats
    let hp_bonus =
        combat.max_hp - base.max_hp;

    let mp_bonus =
        combat.max_mp - base.max_mp;

    let attack_bonus =
        combat.attack - base.attack;

    let defense_bonus =
        combat.defense - base.defense;

    let critical_rate_bonus =
        (combat.critical_rate - base.critical_rate)
            * 100.0;

    let critical_damage_bonus =
        (combat.critical_damage - base.critical_damage)
            * 100.0;
    ...
    span.0 = match kind {
        PlayerStatusValueText::Hp => {
            format!("{} / {:.0}", health.current, combat.max_hp)
        }

        PlayerStatusValueText::Mp => {
            format!("{} / {:.0}", mana.current, combat.max_mp)
        }

        PlayerStatusValueText::Attack => {
            format!("{:.1}", combat.attack)
        }

        PlayerStatusValueText::Defense => {
            format!("{:.1}", combat.defense)
        }

        PlayerStatusValueText::CriticalRate => {
            format!("{:.1}%", combat.critical_rate * 100.0)
        }

        PlayerStatusValueText::CriticalDamage => {
            format!("{:.1}%", combat.critical_damage * 100.0)
        }

        PlayerStatusValueText::AtkAndDefElement => {
            format!("{:?}", atk_and_def_element.0)
        }
    };
}
    ...
    for (kind, mut span) in &mut bonus_query {
        **span = match kind {
            PlayerStatusBonusText::Hp => {
                format!("  (+{:.0})", hp_bonus)
            }

            PlayerStatusBonusText::Mp => {
                format!("  (+{:.0})", mp_bonus)
            }

            PlayerStatusBonusText::Attack => {
                format!("  (+{:.1})", attack_bonus)
            }

            PlayerStatusBonusText::Defense => {
                format!("  (+{:.1})", defense_bonus)
            }

            PlayerStatusBonusText::CriticalRate => {
                format!(
                    "  (+{:.1}%)",
                    critical_rate_bonus,
                )
            ...
            PlayerStatusBonusText::CriticalDamage => {
                format!(
                    "  (+{:.1}%)",
    ...
    for (element, mut span) in &mut exp_query {
        **span = match element {
            ElementExpText::Water => {
                mastery.water.exp.to_string()
            }

            ElementExpText::Fire => {
                mastery.fire.exp.to_string()
            }

            ElementExpText::Wind => {
                mastery.wind.exp.to_string()
            }

            ElementExpText::Earth => {
                mastery.earth.exp.to_string()
            }

            ElementExpText::Inw => {
                mastery.inw.exp.to_string()
            }
        };
    }
}